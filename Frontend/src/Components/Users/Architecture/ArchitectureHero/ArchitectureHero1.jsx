import bg2 from '../../../../assets/architect/bg2.jpg'
import img1 from "../../../../assets/architect/1.jpg"
import img2 from "../../../../assets/architect/2.jpg"
import img3 from "../../../../assets/architect/3.jpg"
import img4 from "../../../../assets/architect/1.jpg"
import img5 from "../../../../assets/architect/2.jpg"
import img6 from "../../../../assets/architect/3.jpg"

function ArchitectureHero1() {
    return (
        <>
        
    {/* ONE */}
    <section className="h-screen overflow-hidden bg-cover bg-center"  style={{ backgroundImage: `url(${bg2})` }}>
      
      <div className="flex items-center justify-between h-full bg-black opacity-85">
        <div className="p-20 flex-1">
          <p className="font-custom text-[5.5rem] leading-none mb-10 text-white">The fastest <br/> on  planet  </p>
          <div className="flex gap-4">
            <button className="font-custom btn bg-teal-600 text-white">Request a call</button>
            <button className="font-custom btn border border-gray-600 font-medium text-white">Get Consultation</button>
          </div>

        </div>
        <div className="flex-1 flex gap-6 justify-end "> 
<img className="img" src={img1} alt="img1" />
<img className="img" src={img2} alt="img2" />
<img className="img" src={img3} alt="img3" />
<img className="img" src={img4} alt="img4" />
<img className="img" src={img5} alt="img5" />
<img className="img" src={img6} alt="img6" />
        </div>
      </div>



    </section>
            
        </>
    )
}

export default ArchitectureHero1
